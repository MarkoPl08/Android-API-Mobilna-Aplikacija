package hr.algebra.lol_projekt.dao
...
private val CREATE_TABLE = "create table $TABLE_NAME( " +
        ...
        "${Item::blurb.name} text not null, " +
        "${Item::image.name} text not null, " +
        "${Item::read.name} integer not null" +
        ")"
private const val DROP_TABLE = "drop table $TABLE_NAME"

class LoLSqlHelper(context: Context?) :
    SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION), LolRepository {
